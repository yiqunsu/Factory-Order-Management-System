...
export default function FormulaTab() {
  ...
  async function handleSave() {
    if (!form.name.trim() || !form.productId) { setError("配方名称和关联产品为必填项"); return; }
  ...
  async function handleDelete() {
    if (!deleting) return;
    const res = await fetch(`/api/formulas/${deleting.id}`, { method: "DELETE" });
                ...
                return (
                  <tr key={f.id} className="hover:bg-blue-50/30 transition-colors group">
                    ...
                    <td className="px-5 py-4">
                      ...
                      {f.source && (
                        <span className="text-xs text-slate-400 mt-0.5 block truncate">衍生自：{f.source.name}</span>
                      )}
                    </td>
                    {/* 关联产品 — 大类 · 产品名 两行分层展示 */}
                    <td className="px-5 py-4">
                    ...
                    <td className="px-5 py-4">
                      {f.materials?.trim() ? (
                        <button
                          onClick={() => setViewingMaterials(f.materials)}
                          className="w-full text-left group/mat"
                        >
                          <span className="block text-xs text-slate-600 leading-relaxed line-clamp-2 group-hover/mat:text-blue-600 transition-colors">
                            {f.materials}
                          </span>
                    ...
                    <td className="px-5 py-4">
                      {f.notes ? (
                        <button
                          onClick={() => setViewingNotes(f.notes)}
                    ...
                    <td className="px-5 py-4">
                      <div className="flex items-center justify-end gap-1.5 opacity-0 group-hover:opacity-100 transition-opacity">
                        ...
                        <button
                          onClick={() => { setDeleting(f); setDeleteDialogOpen(true); }}
