...
export default function MachineTab() {
  ...
  function openEdit(m: Machine) {
    ...
    setForm({
      name: m.name,
      isActive: m.isActive,
      minWidth: String(m.minWidth),
      maxWidth: String(m.maxWidth),
      notes: m.notes ?? "",
      categoryIds: m.categories.map((c) => c.category.id),
      patternIds:  m.patterns.map((p) => p.pattern.id),
    });
    setError("");
    setDialogOpen(true);
  }
  ...
  async function handleSave() {
    ...
    const min = parseFloat(form.minWidth);
    const max = parseFloat(form.maxWidth);
    ...
    try {
      ...
      const res = await fetch(url, {
        ...
        body: JSON.stringify({ ...form, minWidth: min, maxWidth: max }),
      });
  ...
  async function handleDelete() {
    if (!deleting) return;
    const res = await fetch(`/api/machines/${deleting.id}`, { method: "DELETE" });
              ...
              <div key={m.id} className="px-4 py-3.5">
                <div className="flex items-center gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-sm text-slate-500 mt-0.5 tabular-nums">{m.minWidth}–{m.maxWidth} mm</p>
                  </div>
            ...
            <tbody className="divide-y divide-slate-50">
              {machines.map((m) => (
                <tr key={m.id} className="hover:bg-blue-50/30 transition-colors group">
                  ...
                  <td className="px-6 py-4">
                    <button onClick={() => toggleActive(m)} title="点击切换状态" className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-medium transition-colors ${m.isActive ? "bg-emerald-50 text-emerald-700 hover:bg-emerald-100" : "bg-slate-100 text-slate-500 hover:bg-slate-200"}`}>
                      <span className={`w-1.5 h-1.5 rounded-full ${m.isActive ? "bg-emerald-500" : "bg-slate-400"}`} />
                      {m.isActive ? "在用" : "停用"}
                    </button>
                  </td>
                  <td className="px-6 py-4 text-slate-600 tabular-nums">{m.minWidth}–{m.maxWidth} mm</td>
                  <td className="px-6 py-4">
        ...
        <DialogContent className="sm:max-w-lg">
          ...
          <div className="space-y-4 py-2">
            ...
            <div className="space-y-1.5">
              ...
              <div className="flex items-center gap-3">
                <Input
                  type="number"
                  value={form.minWidth}
                  onChange={(e) => setForm({ ...form, minWidth: e.target.value })}
                  placeholder="最小"
                  className="border-slate-200 focus:border-blue-400 focus:ring-blue-400"
                />
            ...
            <CheckboxGroup
              ...
              onChange={(ids) => setForm({ ...form, patternIds: ids })}
            />
