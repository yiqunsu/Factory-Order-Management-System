...
/* ─── 类型 ─── */
interface Category { id: string; name: string; desc: string | null; createdAt: string }
interface Product  { id: string; name: string; categoryId: string; category: Category; createdAt: string }
...
function ProductSection({
  ...
  function openCreate() { setEditing(null); setForm({ name: "", categoryId: categories[0]?.id ?? "" }); setError(""); setDialogOpen(true); }
  function openEdit(p: Product) { setEditing(p); setForm({ name: p.name, categoryId: p.categoryId }); setError(""); setDialogOpen(true); }

  async function handleSave() {
    if (!form.name.trim() || !form.categoryId) { setError("产品名称和所属大类为必填项"); return; }
    setSaving(true); setError("");
    try {
