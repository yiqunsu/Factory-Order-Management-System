...
import {
  SortableContext as _SortableContext, sortableKeyboardCoordinates, arrayMove,
...
export default function KanbanBoard() {
  ...
  async function createTask(machineId: string, orderId: string) {
    ...
    const res = await fetch("/api/production-tasks", {
      ...
      body:    JSON.stringify({ machineId, orderIds: [orderId] }),
    });
    if (!res.ok) { setPendingOrders((prev) => [...prev, order]); return; }
  ...
  async function addOrderToTask(taskId: string, orderId: string) {
    ...
    setMachines((prev) =>
      prev.map((m) => ({
        ...m,
        tasks: m.tasks.map((t) =>
          t.id === taskId ? { ...t, orders: [...t.orders, { ...order, taskId }] } : t
        ),
      }))
    );
  ...
  async function moveOrderToNewTask(orderId: string, fromTaskId: string, machineId: string) {
    ...
    if (fromTask.orders.length === 1) {
      await fetch(`/api/production-tasks/${fromTaskId}`, { method: "DELETE" });
    } else {
      await fetch(`/api/production-tasks/${fromTaskId}`, {
        method:  "PUT",
        headers: { "Content-Type": "application/json" },
        body:    JSON.stringify({ orderIds: fromTask.orders.filter((o) => o.id !== orderId).map((o) => o.id) }),
      });
    }
    ...
    const newTask: KanbanTask = await res.json();
    setMachines((prev) =>
      prev.map((m) =>
        m.id === machineId
  ...
  async function moveOrderBetweenTasks(orderId: string, fromTaskId: string, toTaskId: string) {
      ...
      prev.map((m) => ({
        ...
        tasks: m.tasks
          .map((t) => {
            ...
            if (t.id === toTaskId)   return { ...t, orders: [...t.orders, { ...order, taskId: toTaskId }] };
            return t;
          })
    ...
    await Promise.all([
      fetch(`/api/production-tasks/${toTaskId}`, {
        method:  "PUT",
        headers: { "Content-Type": "application/json" },
        body:    JSON.stringify({ orderIds: toOrderIds }),
      }),
      isLastOrder
        ? fetch(`/api/production-tasks/${fromTaskId}`, { method: "DELETE" })
        : fetch(`/api/production-tasks/${fromTaskId}`, {
            method:  "PUT",
            headers: { "Content-Type": "application/json" },
            body:    JSON.stringify({ orderIds: fromTask.orders.filter((o) => o.id !== orderId).map((o) => o.id) }),
          }),
  ...
  async function deleteTask(taskId: string) {
    ...
    setPendingOrders((prev) =>
      [...prev, ...task.orders.map((o) => ({ ...o, taskId: null, status: "PENDING" }))]
        .sort((a, b) => a.orderNo.localeCompare(b.orderNo))
    );
  ...
  async function moveTaskToMachine(taskId: string, fromMachineId: string, toMachineId: string, position?: number) {
    ...
    setMachines((prev) =>
      prev.map((m) => {
        ...
        if (m.id === toMachineId)   return { ...m, tasks: [...m.tasks, { ...task, machineId: toMachineId, position: newPosition }].sort((a, b) => a.position - b.position) };
        return m;
      })
    );
    ...
    await fetch(`/api/production-tasks/${taskId}`, {
      method:  "PUT",
      headers: { "Content-Type": "application/json" },
      body:    JSON.stringify({ machineId: toMachineId, position: newPosition }),
    });
  }
